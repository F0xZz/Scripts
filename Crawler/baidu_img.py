...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        crawler.start(args.word, args.total_page, args.start_page, args.per_page)  # 抓取关键词为 “乌龟”，总数为 1 页（即总共 1*60=60 张），开始页码为 2
    else:
        # 如果不指定参数，那么程序会按照下面进行执行
        crawler = Crawler(0.05)  # 抓取延迟为 0.05

        crawler.start('乌龟', 10, 2, 30)  # 抓取关键词为 “乌龟”，总数为 1 页，开始页码为 2，每页30张（即总共 2*30=60 张）
        # crawler.start('二次元 乌龟', 10, 1)  # 抓取关键词为 “二次元 乌龟”
